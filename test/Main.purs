...
import Test.Utils (assertSeqEq, test, withPG')
...
main = do
  ...
  void $ launchAff do
    PG.withConnection pool case _ of
      Left pgErr → do
        log ("Error during test database connection: " <> unsafeStringify pgErr)
        log "Have you configured \"purspg\" test database?"
      Right conn → do
...
test' conn msg expected q = do
  test conn msg $ (withPG dbconfig $ query q) >>= case _ of
    Left pgError → throwError $ error $ "Pg error during test execution: " <> unsafeStringify pgError
    Right r → assertSeqEq expected r
